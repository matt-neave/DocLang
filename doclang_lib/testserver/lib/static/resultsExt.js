function Results($scope, $http, $timeout) {
  ...
  self.load_status = function() {
    $http.get('api/extension/status').success(function(data) {
  ...
  $scope.run = function() {
    $scope.status = { status: "loading..." };
    $http.post('api/extension/run').success(function() {
      self.load_status();
    });
  }
  ...
  $scope.run_one = function(index) {
    ...
    $http.post('api/extension/run_one' + tc).success(function() {
      self.load_status();
    });
  };
  ...
  $scope.test_case_source_file = function(source_file) {
    return self.load_file('source_files', '/files/extension', source_file);
  }

  $scope.test_case_binary_file = function(binary_file) {
    return self.load_file('binary_files', '/files/extension/binary', binary_file);
  }
  ...
  $scope.clear_one_file_cache = function(tc) {
    console.log(tc);
    delete $scope.source_files[tc + '.txt'];
